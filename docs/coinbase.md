...
Coinbase 的行情数据根据 [Coinbase](https://docs.pro.coinbase.com/#websocket-feed) 提供的方式，
通过websocket协议，订阅 Coinbase 官方实时推送的行情数据。然后程序将源数据经过适当打包处理，并通过行情事件的形式发布到事件中心。

当前行情服务器能够收集 Coinbase 的行情数据包括：Orderbook(订单薄)、Trade(成交)。
...
```
以上配置表示：订阅 `coinbase` 交易所里，交易对 `BTC/USD` 和 `ETH/USD` 的 `orderbook 订单薄` 和 `trade 成交` 行情数据。
