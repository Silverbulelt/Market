...
Huobi(火币 现货)的行情数据根据 [Huobi官方文档](https://huobiapi.github.io/docs/spot/v1/cn/) 提供的方式，
通过websocket协议，订阅Huobi官方实时推送的行情数据。然后程序将源数据经过适当打包处理，并通过行情事件的形式发布到事件中心。

当前行情服务器能够收集Huobi的行情数据包括：Orderbook(订单薄)、Trade(成交)、Kline(K线)。
